...
VertexArray::VertexArray(const float* pVertexData, unsigned int numVertices, const unsigned int* pIndexData, unsigned int numIndicies)
	: m_numVertices(numVertices), m_numIndicies(numIndicies), m_glVertexBuffer(0), m_glIndexBuffer(0), m_glVertexArray(0)
{
	...
	if (pIndexData)
	{
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_numIndicies * sizeof(unsigned int), pIndexData, GL_STATIC_DRAW);
	}
